...
router.get('/', authenticate, authorize('admin', 'employee'), asyncHandler(async (_req, res) => {
...
router.put('/:id', authenticate, authorize('admin', 'employee'), upload.single('cv'), asyncHandler(async (req, res) => {
  const body = { ...req.body, cv_file: req.file ? `/uploads/${req.file.filename}` : req.body.cv_file };
