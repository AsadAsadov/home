...
router.put('/sections/:id', adminOnly, mediaFields, asyncHandler(async (req, res) => {
  ...
  const row = await prisma.seaBreezeSection.update({
    ...
    data: {
      ...
      imageUrl: bool(req.body.removeImage, false) ? null : (await up(file(req, 'image')) || str(req.body.image_url || req.body.imageUrl, old.imageUrl || '')),
      videoUrl: bool(req.body.removeVideo, false) ? null : (await up(file(req, 'video')) || str(req.body.video_url || req.body.videoUrl, old.videoUrl || '')),
