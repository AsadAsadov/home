...
async function generateProjectBrochurePdf(project) {
  ...
  if (logo) doc.image(page, logo, MARGIN, 720, 128, 55);
  else doc.text(page, BRAND, MARGIN, 744, 22, { bold: true, color: BRAND_COLOR });
  ...
  page = doc.addPage();
  if (logo) doc.image(page, logo, MARGIN, 764, 92, 39);
  else doc.text(page, BRAND, MARGIN, 784, 16, { bold: true, color: BRAND_COLOR });
  let y = addSectionTitle(doc, page, 'Project information', 730);
  const info = [
  ...
  for (let i = 0; i < info.length; i += 2) {
    ...
    y -= 56;
    if (y < 100) { footer(doc, page, pageNo++); page = doc.addPage(); y = addSectionTitle(doc, page, 'Project information', 770); }
  }
  footer(doc, page, pageNo++);

  page = doc.addPage();
  y = addSectionTitle(doc, page, 'Apartments and pricing', 770);
  const pricing = [
  ...
  features.slice(0, 24).forEach((item) => {
    ...
    y -= 28;
    if (y < 65) { footer(doc, page, pageNo++); page = doc.addPage(); y = addSectionTitle(doc, page, 'Infrastructure and features', 770); }
  });
  footer(doc, page, pageNo++);
  ...
  if (images.length) {
    for (let i = 0; i < images.length; i += 2) {
      page = doc.addPage();
      y = addSectionTitle(doc, page, i === 0 ? 'Image gallery' : 'Image gallery (continued)', 770);
