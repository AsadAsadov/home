...
function projectData(p) {
  ...
  return {
    ...
    category: p.category || p.type || p.projectType || null,
    zone: p.zone || null,
    deliveryDate: p.delivery_date || p.deliveryDate || p.year || null,
    coastline: p.coastline || null,
    seaDistance: p.sea_distance || p.seaDistance || null,
    buildingCount: p.building_count || p.buildingCount || null,
    floorCount: p.floor_count || p.floorCount || p.floors || null,
    area: p.area || null,
    apartmentCount: p.apartment_count || p.apartmentCount || p.apartments || null,
    parkingSpaces: p.parking_spaces || p.parkingSpaces || null,
    repairStatus: p.repair_status || p.repairStatus || p.repair || null,
    apartmentFormats: p.apartment_formats || p.apartmentFormats || null,
    apartmentAreas: p.apartment_areas || p.apartmentAreas || null,
    areaRange: p.area_range || p.areaRange || p.area || null,
    pricePerM2: p.price_per_m2 || p.pricePerM2 || null,
    totalPrice: p.total_price || p.totalPrice || null,
    bankMortgage: p.bank_mortgage || p.bankMortgage || null,
    internalCredit: p.internal_credit || p.internalCredit || null,
    downPayment: p.down_payment || p.downPayment || null,
